...
func TestShowPageAction_WritesMendix9PageVariable(t *testing.T) {
	action := &microflows.ShowPageAction{
		BaseElement: model.BaseElement{ID: "test-action-id"},
		PageName:    "Sales.Product_NewEdit",
		PageParameterMappings: []*microflows.PageParameterMapping{
	...
	if len(mappings) < 2 || mappings[0] != int32(2) {
		t.Fatalf("ParameterMappings marker = %#v, want int32(2)", mappings)
	}
	...
	if variable == nil {
		t.Fatal("Variable is nil; Mendix 9 rejects null page parameter mapping variables")
	}
