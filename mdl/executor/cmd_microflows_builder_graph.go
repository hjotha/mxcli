...
func planMicroflowCallOutputDeclarations(stmts []ast.MicroflowStatement) map[*ast.CallMicroflowStmt]bool {
	...
	outputCounts := countMicroflowCallOutputs(stmts)
	referencedVars := referencedVariableSet(stmts)
	seen := make(map[string]bool)
	cloneSeen := func(src map[string]bool) map[string]bool {
	...
	var walk func([]ast.MicroflowStatement)
	var walkWithSeen func([]ast.MicroflowStatement, map[string]bool)
	walk = func(body []ast.MicroflowStatement) {
		walkWithSeen(body, seen)
	}
	walkWithSeen = func(body []ast.MicroflowStatement, localSeen map[string]bool) {
		for _, stmt := range body {
			switch s := stmt.(type) {
			case *ast.CallMicroflowStmt:
				if s.OutputVariable != "" {
					if outputCounts[s.OutputVariable] > 1 && !referencedVars[s.OutputVariable] {
						declare[s] = false
					} else {
						declare[s] = !localSeen[s.OutputVariable]
					}
					localSeen[s.OutputVariable] = true
				}
				if s.ErrorHandling != nil {
					walkWithSeen(s.ErrorHandling.Body, localSeen)
				}
			case *ast.IfStmt:
				walkWithSeen(s.ThenBody, cloneSeen(localSeen))
				walkWithSeen(s.ElseBody, cloneSeen(localSeen))
			case *ast.LoopStmt:
				walkWithSeen(s.Body, cloneSeen(localSeen))
			case *ast.WhileStmt:
				walkWithSeen(s.Body, cloneSeen(localSeen))
			case *ast.CreateObjectStmt:
				if s.ErrorHandling != nil {
