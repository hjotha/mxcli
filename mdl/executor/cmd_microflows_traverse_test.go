...
func TestTraverseFlow_SkipsNoOpSplitWhoseBranchesOnlyJoin(t *testing.T) {
	e := newTestExecutor()
	...
	activityMap := map[model.ID]microflows.MicroflowObject{
		...
		mkID("create"): &microflows.ActionActivity{
			BaseActivity: microflows.BaseActivity{BaseMicroflowObject: mkObj("create")},
			Action: &microflows.CreateObjectAction{
				EntityQualifiedName: "SampleSync.ProcessingResult",
		...
		mkID("split"): &microflows.ExclusiveSplit{
			BaseMicroflowObject: mkObj("split"),
			Caption:             "Event Type?",
			SplitCondition:      &microflows.ExpressionSplitCondition{Expression: "$EventType"},
		},
		mkID("merge"): &microflows.ExclusiveMerge{BaseMicroflowObject: mkObj("merge")},
		mkID("end"):   &microflows.EndEvent{BaseMicroflowObject: mkObj("end"), ReturnValue: "$ProcessingResult"},
	}

	flowsByOrigin := map[model.ID][]*microflows.SequenceFlow{
		mkID("start"):  {mkFlow("start", "create")},
		mkID("create"): {mkFlow("create", "split")},
		mkID("split"): {
			mkBranchFlow("split", "merge", microflows.EnumerationCase{Value: "CREATE"}),
			mkBranchFlow("split", "merge", microflows.EnumerationCase{Value: "UPDATE"}),
			mkBranchFlow("split", "merge", microflows.EnumerationCase{Value: "DELETE"}),
			mkBranchFlow("split", "merge", microflows.EnumerationCase{Value: ""}),
		},
		mkID("merge"): {mkFlow("merge", "end")},
	}
	...
	if strings.Contains(joined, "if $EventType then") || strings.Contains(joined, "Event Type?") {
		t.Fatalf("no-op split with all branches joining should not be emitted as authorable MDL:\n%s", joined)
	}
	assertContains(t, joined, "$ProcessingResult = create SampleSync.ProcessingResult;")
	assertContains(t, joined, "return $ProcessingResult;")
}

func TestTraverseFlow_EnumSplitPreservesEnumerationCases(t *testing.T) {
	e := newTestExecutor()
	...
	activityMap := map[model.ID]microflows.MicroflowObject{
		...
		mkID("delete_log"): &microflows.ActionActivity{
			BaseActivity: microflows.BaseActivity{BaseMicroflowObject: mkObj("delete_log")},
			Action:       &microflows.LogMessageAction{LogLevel: "Info", LogNodeName: "'Sample'", MessageTemplate: &model.Text{Translations: map[string]string{"en_US": "delete"}}},
		},
		mkID("empty_log"): &microflows.ActionActivity{
			BaseActivity: microflows.BaseActivity{BaseMicroflowObject: mkObj("empty_log")},
			Action:       &microflows.LogMessageAction{LogLevel: "Info", LogNodeName: "'Sample'", MessageTemplate: &model.Text{Translations: map[string]string{"en_US": "empty"}}},
		},
		mkID("merge"): &microflows.ExclusiveMerge{BaseMicroflowObject: mkObj("merge")},
		mkID("end"):   &microflows.EndEvent{BaseMicroflowObject: mkObj("end")},
	}
	...
	flowsByOrigin := map[model.ID][]*microflows.SequenceFlow{
		...
		mkID("split"): {
			mkBranchFlow("split", "merge", microflows.EnumerationCase{Value: "CREATE"}),
			mkBranchFlow("split", "merge", microflows.EnumerationCase{Value: "UPDATE"}),
			mkBranchFlow("split", "delete_log", microflows.EnumerationCase{Value: "DELETE"}),
			mkBranchFlow("split", "empty_log", microflows.EnumerationCase{Value: "(empty)"}),
		},
		mkID("delete_log"): {mkFlow("delete_log", "merge")},
		mkID("empty_log"):  {mkFlow("empty_log", "merge")},
		mkID("merge"):      {mkFlow("merge", "end")},
	}
	splitMergeMap := map[model.ID]model.ID{mkID("split"): mkID("merge")}
	...
	joined := strings.Join(lines, "\n")
	if strings.Contains(joined, "if $EventType then") {
		t.Fatalf("enum ExclusiveSplit must not be described as a boolean IF:\n%s", joined)
	}
	assertContains(t, joined, "split enum $EventType")
	assertContains(t, joined, "case CREATE, UPDATE")
	assertContains(t, joined, "case DELETE")
	assertContains(t, joined, "case (empty)")
	assertContains(t, joined, "end split;")
}

func TestTraverseFlow_EnumSplitSharedTailStaysAfterTerminalCase(t *testing.T) {
	e := newTestExecutor()
	...
	activityMap := map[model.ID]microflows.MicroflowObject{
		...
		mkID("create_log"): &microflows.ActionActivity{
			BaseActivity: microflows.BaseActivity{BaseMicroflowObject: mkObj("create_log")},
			Action:       &microflows.LogMessageAction{LogLevel: "Info", LogNodeName: "'Sample'", MessageTemplate: &model.Text{Translations: map[string]string{"en_US": "create"}}},
		},
		mkID("update_log"): &microflows.ActionActivity{
			BaseActivity: microflows.BaseActivity{BaseMicroflowObject: mkObj("update_log")},
			Action:       &microflows.LogMessageAction{LogLevel: "Info", LogNodeName: "'Sample'", MessageTemplate: &model.Text{Translations: map[string]string{"en_US": "update"}}},
		},
		mkID("shared_tail"): &microflows.ActionActivity{
			BaseActivity: microflows.BaseActivity{BaseMicroflowObject: mkObj("shared_tail")},
			Action:       &microflows.LogMessageAction{LogLevel: "Info", LogNodeName: "'Sample'", MessageTemplate: &model.Text{Translations: map[string]string{"en_US": "shared tail"}}},
		},
		mkID("empty_end"): &microflows.EndEvent{BaseMicroflowObject: mkObj("empty_end")},
		mkID("end"):       &microflows.EndEvent{BaseMicroflowObject: mkObj("end")},
	}
	...
	flowsByOrigin := map[model.ID][]*microflows.SequenceFlow{
		mkID("start"): {mkFlow("start", "split")},
		mkID("split"): {
			mkBranchFlow("split", "create_log", microflows.EnumerationCase{Value: "CREATE"}),
			mkBranchFlow("split", "update_log", microflows.EnumerationCase{Value: "UPDATE"}),
			mkBranchFlow("split", "empty_end", microflows.EnumerationCase{Value: "(empty)"}),
		},
		mkID("create_log"):  {mkFlow("create_log", "shared_tail")},
		mkID("update_log"):  {mkFlow("update_log", "shared_tail")},
		mkID("shared_tail"): {mkFlow("shared_tail", "end")},
	}
	splitMergeMap := findSplitMergePointsForGraph(nil, activityMap, flowsByOrigin)
	if got := splitMergeMap[mkID("split")]; got != mkID("shared_tail") {
		t.Fatalf("enum split paired with %q, want shared tail %q", got, mkID("shared_tail"))
	}
	...
	joined := strings.Join(lines, "\n")
	if count := strings.Count(joined, "shared tail"); count != 1 {
		t.Fatalf("shared tail should be emitted once, got %d:\n%s", count, joined)
	}
	endSplit := strings.Index(joined, "end split;")
	sharedTail := strings.Index(joined, "shared tail")
	if endSplit < 0 || sharedTail < 0 || sharedTail < endSplit {
		t.Fatalf("shared tail should be emitted after enum split closes:\n%s", joined)
	}
	assertContains(t, joined, "case CREATE")
	assertContains(t, joined, "case UPDATE")
	assertContains(t, joined, "case (empty)")
}
...
func TestTraverseFlow_LoopBodyUsesNestedAnnotationFlows(t *testing.T) {
	...
	bodyLog := &microflows.ActionActivity{
		BaseActivity: microflows.BaseActivity{BaseMicroflowObject: microflows.BaseMicroflowObject{
			BaseElement: model.BaseElement{ID: mkID("body-log")},
			Position:    model.Point{X: 300, Y: 100},
		}},
		Action: &microflows.LogMessageAction{LogLevel: "Info", LogNodeName: "'Synthetic'", MessageTemplate: &model.Text{Translations: map[string]string{"en_US": "active"}}},
	}
	...
	loop := &microflows.LoopedActivity{
		...
		ObjectCollection: &microflows.MicroflowObjectCollection{
			Objects: []microflows.MicroflowObject{split, bodyLog, note},
			Flows: []*microflows.SequenceFlow{
				mkBranchFlow("split", "body-log", &microflows.ExpressionCase{Expression: "true"}),
				mkBranchFlow("split", "loop", &microflows.ExpressionCase{Expression: "false"}),
				mkFlow("body-log", "loop"),
			},
	...
	var lines []string
	e.traverseFlow(mkID("start"), activityMap, flowsByOrigin, nil, make(map[model.ID]bool), nil, nil, &lines, 0, nil, 0, buildAnnotationsByTarget(&microflows.MicroflowObjectCollection{}))

	out := strings.Join(lines, "\n")
	if !strings.Contains(out, "@annotation 'nested split note'") {
		t.Fatalf("expected nested loop annotation in output:\n%s", out)
	}
	if strings.Index(out, "@annotation 'nested split note'") > strings.Index(out, "if $Item/IsActive then") {
		t.Fatalf("annotation should be emitted before the nested split:\n%s", out)
	}
}
