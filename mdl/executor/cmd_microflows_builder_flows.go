...
func newHorizontalFlowWithEnumCase(originID, destinationID model.ID, caseValue string) *microflows.SequenceFlow {
	flow := newHorizontalFlow(originID, destinationID)
	setEnumCaseValue(flow, caseValue)
	return flow
}
...
func newDownwardFlowWithEnumCase(originID, destinationID model.ID, caseValue string) *microflows.SequenceFlow {
	flow := newDownwardFlowWithCase(originID, destinationID, caseValue)
	setEnumCaseValue(flow, caseValue)
	return flow
}

func setEnumCaseValue(flow *microflows.SequenceFlow, caseValue string) {
	if caseValue == "" {
		flow.CaseValue = microflows.NoCase{BaseElement: model.BaseElement{ID: model.ID(types.GenerateID())}}
		return
	}
	flow.CaseValue = microflows.EnumerationCase{
		BaseElement: model.BaseElement{ID: model.ID(types.GenerateID())},
